...
public class DictionaryHelper extends SQLiteOpenHelper {
  ...
  private static final File dbFile = new File("/data/data/com.buweima.trime/databases/", DB_NAME);
  ...
  private boolean importDict(InputStream is) {
    ...
    try {
      ...
      db.execSQL(String.format("CREATE VIRTUAL TABLE %s USING fts3(hz, pya, pyb, pyc, pyz)", table));  /* abcz*/

      ContentValues initialValues = new ContentValues(2); /* abcz*/
      int max = is.available();
      int progress = 0;
      int count = 0;
      while ((line = br.readLine()) != null) {
        ...
        String[] s = line.split("\t");
        if (s.length < 5) continue; /* abcz*/
        initialValues.put("hz", s[0]);
        initialValues.put("pya", s[1]);
        initialValues.put("pyb", s[2]);/* abcz*/
        initialValues.put("pyc", s[3]);/* abcz*/
        initialValues.put("pyz", s[4]); /* abcz*/       
