...
public class Dictionary {
  ...
  public Cursor getWord(CharSequence code) {
    ...
    if (cursor == null && !fullPyOn) {
      s = s.replace(" OR", "* OR") + "*";
      cursor = query(sql + " limit 20000", new String[]{s});
    }
    //Log.e("kyle", "word end");
    return cursor;
  }
  ...
  private Cursor getPhrase(CharSequence code) {
    ...
    String sql = String.format("select %s from %s where py match ? limit 20000", getQueryCol(), table);
  ...
  public Cursor getAssociation(CharSequence code) {
    ...
    String s = code.toString();
    int len = s.length();
    String sqlFormat = "select distinct substr(hz,%d) from %s where hz match '^%s*' and length(hz) > %d limit 20000";
  ...
  public String toSC(String text) {
    if (!isSC()) return text;
    Cursor cursor =  query("select t from opencc where s match ?", new String[]{text});
    if (cursor != null) {
    ...
    for (String i: text.split("\\B")) {
      cursor = query("select t from opencc where s match ?", new String[]{i});
  ...
  public int getCandTextSize() {
    return Integer.parseInt(preferences.getString("pref_cand_font_size", "28"));
  }
